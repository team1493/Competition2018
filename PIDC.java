...
public class PIDC {
private NavXSensors navx;
private SerialSonar sonar;
private FalconDrive falconDrive;

private final double TICKS_PER_INCH = 57.295;  // 6 in diam wheel, 1080 ticks per rev
...
private double TOLERANCE_STRAIGHTENC_GYRO = 30;
private double TIMEONTARGETSET_STRAIGHTENC_GYRO = .05;
...
// ** add while Auto mode ?
	public PIDC(FalconDrive falconDrive1,NavXSensors navx1,SerialSonar sonar1,Joystick joya) {
...
//**************************************	
	public void rotate(double target1) {
		...
		while(timeontarget<TIMEONTARGETSET_TURN && joy0.getRawAxis(1)<.5) {
			...
			falconDrive.arcadeDrive(0, -rotateValue,false,2);	
			falconDrive.display();
...
//**************************************		
	public void driveStraightGyro(double target1) {
		...
		while(timeontarget<TIMEONTARGETSET_STRAIGHTENC_GYRO && joy0.getRawAxis(1)<.5) {
			...
			falconDrive.arcadeDrive(straightValue, -rotateValue,false,2);	
			falconDrive.display();
	...
	//**************************************		
		public void driveStraightSonarGyro(double target1) {
			target = target1*TICKS_PER_INCH;  // sonar class converts mm outout to inches 
			Thread t = new Thread(() -> {
			double position1=0,position2=0;
			double angle1=0,targetangle;
			double err1=0,err2=0,errangle=0; 
		...
		//**************************************	  
		// Drive straight a using gyro until a bump (jerk)
		// Use the navx accelerometer to calculate jerk
		//**************************************		
			public void driveStraightBump(double target1) {
				target = target1*TICKS_PER_INCH;
				Thread t = new Thread(() -> {
				double position1=0;
				double angle1=0,targetangle,errangle;
				double rotateValue,straightValue;
				boolean velmode=false;
				if(falconDrive.getmode()==VELOCITY) velmode = true;
				angle1 = navx.getAngle();
				...
				target=position1+target;
				errangle=targetangle-angle1;
				while(falconDrive.getPositionRight() < target && joy0.getRawAxis(1)<.5) {
					angle1=navx.getAngle();
					errangle=targetangle-angle1;			
					straightValue= -0.5;
					rotateValue= limit(errangle*KPANGLE_SRAIGHTENC_GYRO,sg_MAX_ROTATE_POWER);
				...
				falconDrive.arcadeDrive(0.3, 0, true, 2);
				while (falconDrive.getVelocityRight()>10) {
					angle1=navx.getAngle();
					errangle=targetangle-angle1;			
					straightValue= -0.35;
					rotateValue= limit(errangle*KPANGLE_SRAIGHTENC_GYRO,sg_MAX_ROTATE_POWER);
...
//						KP_STRAIGHTSONAR_GYRO=SmartDashboard.getNumber("DB/Slider 0", 0);
//						KI_STRAIGHTSONAR_GYRO=SmartDashboard.getNumber("DB/Slider 1", 0);
//						KD_STRAIGHTSONAR_GYRO=SmartDashboard.getNumber("DB/Slider 2", 0);
//						KIZONE_STRAIGHTSONAR_GYRO=SmartDashboard.getNumber("DB/Slider 3", 0);
